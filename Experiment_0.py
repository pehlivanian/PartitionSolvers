...
rp10_title = 'Confusion matrix - 2 true risk partitions, t = 10'
rp3_label  = ['1','2','3']
rp3_title  = 'Confusion matrix - 2 true risk partitions, t = 3'
rp2_label  = ['1','2']
rp2_title  = 'confusion matrix - 2 true risk partitions, t = 2'

fig = plot_confusion(confusion_rp10.astype('int'), ['1','2'],rp10_label, title=rp10_title)
plot.pause(1e-3)
plot.close()
fig.savefig('Confusion_matrix_rp10_exp0.pdf')
fig = plot_confusion(confusion_rp3.astype('int'), ['1','2'],rp3_label, title=rp3_title)
fig.show
plot.close()
fig.savefig('Confusion_matrix_rp3_exp0.pdf')
fig = plot_confusion(confusion_rp2.astype('int'), ['1','2'],rp2_label, title=rp2_title)
plot.pause(1e-3)
plot.close()
fig.savefig('Confusion_matrix_rp2_exp0.pdf')


# Plot ranking quality
import matplotlib.pyplot as plot
exp0_ranking_quality_df = pd.read_csv("exp0_ranking_quality.csv")
exp0_ranking_quality_df = exp0_ranking_quality_df.set_index('epsilon')
exp0_ranking_quality_df = exp0_ranking_quality_df.rename(columns={'rp2': 't = 2', 'rp3': 't = 3', 'rp10': 't = 10'})
exp0_ranking_quality_df.plot(kind='line', grid=True, style='.-', title='Ranking quality - 2 risk partitions', ylabel='ranking quality', xlabel='signal strength (epsilon)')
plot.pause(1e-3)
# plot.savefig('ranking_quality_exp0.pdf')
plot.close()
...
import matplotlib.pyplot as plot
exp0_detection_power_df = pd.read_csv("exp0_detection_power.csv")
exp0_detection_power_df = exp0_detection_power_df.set_index('epsilon')
exp0_detection_power_df = exp0_detection_power_df.rename(columns={'rp2': 't = 2', 'rp3': 't = 3', 'rp10': 't = 10'})
exp0_detection_power_df.plot(kind='line', grid=True, style='.-', title='Detection power - 2 risk partitions', ylabel='detection power', xlabel='signal strength (epsilon)')
plot.pause(1e-3)
# plot.savefig('detection_power_exp0.pdf')
plot.close()
