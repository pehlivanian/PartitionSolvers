...
for i in range(num_null_experiments):
    ...
    result_rp20 = solverSWIG_DP.OptimizerSWIG(20,a,b,1,True,True)()[1]
    result_rp40 = solverSWIG_DP.Optimzer_SWIG(40,a,b,1,True,True)()[1]
    ...
    result_mcd20 = solverSWIG_DP.OptimizerSWIG(20,a,b,1,False,True)()[1]
    result_mcd40 = solverSWIG_DP.OptimizerSWIG(40,a,b,1,False,True)()[1]
    null_scores[i,:] = [result_rp2,result_rp3,result_rp10,result_rp20, result_rp40,
                        result_mcd2,result_mcd3,result_mcd10,result_mcd20,result_mcd40]
null_scores_df = pd.DataFrame(null_scores,columns=['rp2','rp3','rp10','rp20','rp40',
                                                   'mcd2','mcd3','mcd10','mcd20','mcd40'])
...
null_scores_rp20 = null_scores_df.loc[:,'rp20']
null_scores_rp40 = null_scores_df.loc[:,'rp40']
null_scores_mcd2 = null_scores_df.loc[:,'mcd2']
null_scores_mcd3 = null_scores_df.loc[:,'mcd3']
null_scores_mcd10 = null_scores_df.loc[:,'mcd10']
null_scores_mcd20 = null_scores_df.loc[:,'mcd20']
null_scores_mcd40 = null_scores_df.loc[:,'mcd40']
...
thresholds_rp20 = bootstrap_95th_percentile(null_scores_rp20)
thresholds_rp40 = bootstrap_95th_percentile(null_scores_rp40)
thresholds_mcd2 = bootstrap_95th_percentile(null_scores_mcd2)
thresholds_mcd3 = bootstrap_95th_percentile(null_scores_mcd3)
thresholds_mcd10 = bootstrap_95th_percentile(null_scores_mcd10)
thresholds_mcd20 = bootstrap_95th_percentile(null_scores_mcd20)
thresholds_mcd40 = bootstrap_95th_percentile(null_scores_mcd40)
thresholds_df = pd.DataFrame({'rp2':thresholds_rp2,
                              'rp3':thresholds_rp3,
                              'rp10':thresholds_rp10,
                              'rp20':thresholds_rp20,
                              'rp40':thresholds_rp40,                              
                              'mcd2':thresholds_mcd2,
                              'mcd3':thresholds_mcd3}
                              'mcd10':thresholds_mcd10,
                              'mcd20':thresholds_mcd20,
                              'mcd40':thresholds_mcd40,                             
                             )
...
for j in range(num_epsilon_values):
    ...
    ranking_quality_rp20 = 0
    ranking_quality_rp40 = 0
    for i in range(num_experiments_per_epsilon):
        ...
        all_rp20 = solverSWIG_DP.OptimizerSWIG(20,a,b,1,True,True)()
        all_rp40 = solverSWIG_DP.OptimizerSWIG(40,a,b,1,True,True)()
        score_rp2 = all_rp2[1]
        score_rp3 = all_rp3[1]
        score_rp10 = all_rp10[1]
        score_rp20 = all_rp20[1]
        score_rp40 = all_rp40[1]
        confusion_rp2 = np.zeros([3,2])
        confusion_rp3 = np.zeros([3,3])
        confusion_rp10 = np.zeros([3,10])
        confusion_rp20 = np.zeros([3,20])
        confusion_rp40 = np.zeros([3,40])
        ...
        for k in range(20):
            thepartition = np.array(all_rp20[0][k])
            confusion_rp20[0,k] = len(thepartition[thepartition <= n1])
            confusion_rp20[1,k] = len(thepartition[(thepartition >= n1+1) & (thepartition <= n1+n2)])
            confusion_rp20[2,k] = len(thepartition[thepartition >= n1+n2+1])
        ranking_quality_rp20 += ranking_quality(confusion_rp20)

        for k in range(40):
            thepartition = np.array(all_rp40[0][k])
            confusion_rp40[0,k] = len(thepartition[thepartition <= n1])
            confusion_rp40[1,k] = len(thepartition[(thepartition >= n1+1) & (thepartition <= n1+n2)])
            confusion_rp40[2,k] = len(thepartition[thepartition >= n1+n2+1])
        ranking_quality_rp40 += ranking_quality(confusion_rp40)
        ...
        exp1_scores[theindex,4] = score_rp20
        exp1_scores[theindex,5] = score_rp40
    ...
    exp1_ranking_quality[j,4] = ranking_quality_rp20 / num_experiments_per_epsilon
    exp1_ranking_quality[j,5] = ranking_quality_rp40 / num_experiments_per_epsilon
    
exp1_scores_df = pd.DataFrame(exp1_scores,columns=['epsilon','rp2','rp3','rp10'])
exp1_scores_df.to_csv("exp1_scores.csv",index_label='i')
...
exp1_ranking_quality_df = pd.read_csv("exp1_ranking_quality.csv")
exp1_ranking_quality_df = exp1_ranking_quality_df.set_index('epsilon')
...
exp1_scores_df = pd.read_csv("exp1_scores.csv")
num_epsilon_values = 11
...
detection_power_df.to_csv("exp1_detection_power.csv",index=False)
#
########################################################################

# Plot detection power
import matplotlib.pyplot as plot
exp1_detection_power_df = pd.read_csv("exp1_detection_power.csv")
exp1_detection_power_df = exp1_detection_power_df.set_index('epsilon')
