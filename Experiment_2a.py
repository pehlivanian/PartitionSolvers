...
for j in range(num_epsilon_values):
    ...
    for i in range(num_experiments_per_epsilon):
        b = rng.poisson(100,size=500).astype(float)
        q = np.concatenate([np.full(50,1.0-theepsilon),
                            np.full(50,1.0-4.*theepsilon/5.),
                            np.full(50,1.0-3.*theepsilon/5.),
                            np.full(50,1.0-2.*theepsilon/5.),
                            np.full(50,1.0-1.*theepsilon/5.),
                            np.full(50,1.0+1.*theepsilon/5.),
                            np.full(50,1.0+2.*theepsilon/5.),
                            np.full(50,1.0+3.*theepsilon/5.),
                            np.full(50,1.0+4.*theepsilon/5.),
                            np.full(50,1.0+theepsilon)])
        ...
        for k in range(2):
            ...
            for m in range(10):
                confusion_rp2[m,k] = len(thepartition[(thepartition >= 50*m) & (thepartition < 50*(m+1))])
        ranking_quality_rp2 += ranking_quality(confusion_rp2)
            
        for k in range(3):
            thepartition = np.array(all_rp3[0][k])
            for m in range(10):
                confusion_rp3[m,k] = len(thepartition[(thepartition >= 50*m) & (thepartition < 50*(m+1))])
        ranking_quality_rp3 += ranking_quality(confusion_rp3)

        for k in range(10):
            thepartition = np.array(all_rp10[0][k])
            for m in range(10):
                confusion_rp10[m,k] = len(thepartition[(thepartition >= 50*m) & (thepartition < 50*(m+1))])
...
exp2_scores_df.to_csv("exp2a_scores.csv",index_label='i')
exp2_ranking_quality_df = pd.DataFrame(exp2_ranking_quality,columns=['epsilon','rp2','rp3','rp10'])
exp2_ranking_quality_df.to_csv("exp2a_ranking_quality.csv",index=False)
########################################################################

# Plot ranking quality
import matplotlib.pyplot as plot
exp2a_ranking_quality_df = pd.read_csv("exp2a_ranking_quality.csv")
exp2a_ranking_quality_df = exp2a_ranking_quality_df.set_index('epsilon')
exp2a_ranking_quality_df = exp2a_ranking_quality_df.rename(columns={'rp2': 't = 2', 'rp3': 't = 3', 'rp10': 't = 10'})
exp2a_ranking_quality_df.plot(kind='line', grid=True, style='.-', title='Ranking quality - 10 risk partitions', ylabel='ranking quality', xlabel='signal strength (epsilon)')
plot.pause(1e-3)
plot.savefig('ranking_quality_exp2a.pdf')

########################################################################
# Compute detection power for experiment 2
#
null_thresholds_df = pd.read_csv("null_thresholds.csv")
exp2_scores_df = pd.read_csv("exp2a_scores.csv")
num_epsilon_values = 11
...
detection_power_df.to_csv("exp2a_detection_power.csv",index=False)
#
########################################################################

# Plot detection power
import matplotlib.pyplot as plot
exp2a_detection_power_df = pd.read_csv("exp2a_detection_power.csv")
exp2a_detection_power_df = exp2a_detection_power_df.set_index('epsilon')
exp2a_detection_power_df = exp2a_detection_power_df.rename(columns={'rp2': 't = 2', 'rp3': 't = 3', 'rp10': 't = 10'})
exp2a_detection_power_df.plot(kind='line', grid=True, style='.-', title='Detection power - 10 risk partitions', ylabel='detection power', xlabel='signal strength (epsilon)')
plot.pause(1e-3)
plot.savefig('detection_power_exp2a.pdf')
